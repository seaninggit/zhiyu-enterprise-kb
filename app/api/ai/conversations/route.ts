...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (id) {
      ...
      const messages = await db.prepare("SELECT m.*,f.helpful,f.reason FROM ai_messages m LEFT JOIN ai_answer_feedback f ON f.query_log_id=m.query_log_id AND f.user_id=m.user_id WHERE m.conversation_id=? AND m.user_id=? ORDER BY m.id ASC LIMIT 100").bind(id, ctx.userId).all();
