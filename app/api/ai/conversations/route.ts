...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (id) {
      ...
      const messages = await db.prepare("SELECT m.*,f.helpful,f.reason,q.model FROM ai_messages m LEFT JOIN ai_answer_feedback f ON f.query_log_id=m.query_log_id AND f.user_id=m.user_id LEFT JOIN ai_query_logs q ON q.id=m.query_log_id WHERE m.conversation_id=? AND m.user_id=? ORDER BY m.sequence_no ASC,m.id ASC LIMIT 100").bind(id, ctx.userId).all();
      return ok({ conversation, messages: messages.results }, rid);
    }
    const conversations = await db.prepare("SELECT c.*, (SELECT content FROM ai_messages m WHERE m.conversation_id=c.id ORDER BY m.sequence_no DESC,m.id DESC LIMIT 1) AS last_message FROM ai_conversations c WHERE c.user_id=? AND c.status='ACTIVE' ORDER BY c.update_time DESC LIMIT 30").bind(ctx.userId).all();
