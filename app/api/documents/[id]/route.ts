...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const manager=canManageDepartment(ctx,Number(doc.dept_id)); const responsible=Number(doc.owner_user_id||doc.create_user_id)===ctx.userId;const privileged=manager||responsible;
    const versions = privileged?await db.prepare("SELECT * FROM document_versions WHERE document_id=? ORDER BY version DESC").bind(id).all():doc.published_version?await db.prepare("SELECT id,document_id,version,title,change_note,operator,create_time FROM document_versions WHERE document_id=? AND version<=? ORDER BY version DESC").bind(id,Number(doc.published_version)).all():{results:[]};
    ...
    const baseDoc={...doc,tags:tagRow?.tags||"",content:doc.content||doc.extracted_text||""};const visibleDoc=!manager&&!responsible&&doc.status!=="ARCHIVED_ACTIVE"&&doc.published_version?{...baseDoc,title:doc.published_title,summary:doc.published_summary||String(doc.published_content||"").slice(0,180),content:doc.published_content||"",extracted_text:doc.published_content||"",version:doc.published_version,status:"ARCHIVED_ACTIVE"}:baseDoc;
    const approvals=privileged?await db.prepare("SELECT a.*,applicant.display_name applicant,u.display_name approver FROM approval_records a LEFT JOIN users applicant ON applicant.id=a.applicant_user_id LEFT JOIN users u ON u.id=a.approver_user_id WHERE a.document_id=? ORDER BY a.create_time ASC,a.id ASC").bind(id).all():{results:[]};
    const approvalRoute=privileged?await db.prepare(`SELECT ai.id instance_id,ai.route_type,ai.status instance_status,ai.document_version,ai.current_stage,ai.create_time instance_create_time,ai.complete_time,submitter.display_name submitted_by,modifier.display_name modifier,s.stage_no,s.duty_code,s.status,s.comment,s.action_time,u.display_name assignee,au.display_name action_user FROM approval_instances ai JOIN approval_steps s ON s.instance_id=ai.id JOIN users u ON u.id=s.assignee_user_id JOIN users submitter ON submitter.id=ai.submitted_by JOIN users modifier ON modifier.id=ai.modifier_user_id LEFT JOIN users au ON au.id=s.action_user_id WHERE ai.document_id=? ORDER BY ai.id DESC,s.stage_no`).bind(id).all():{results:[]};
    ...
    const ownerCandidates=manager?(await db.prepare(`SELECT DISTINCT u.id,u.display_name name FROM users u JOIN user_departments ud ON ud.user_id=u.id AND ud.dept_id=? WHERE u.status='ACTIVE' AND u.id<>? AND EXISTS(SELECT 1 FROM user_roles ur JOIN role_permissions rp ON rp.role_id=ur.role_id JOIN permissions p ON p.id=rp.permission_id WHERE ur.user_id=u.id AND p.code='knowledge:edit') ORDER BY u.display_name`).bind(doc.dept_id,Number(doc.owner_user_id||doc.create_user_id)).all()).results:[];
    ...
    return ok({ document: visibleDoc, versions: versions.results, approvals:approvals.results, approvalRoute:approvalRoute.results, acl:acl.results,permissionPrincipals,ownerCandidates,spacePermissions,capabilities:{canEdit:editable,canManage:manager,editMode:"STANDARD"},subscribed:Boolean(subscription?.is_active) }, rid);
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const payload = await request.json() as { title?: string; content?: string; summary?: string; changeNote?:string }; const title = safeText(payload.title || doc.title, 200); const content = safeText(payload.content || doc.content, 500000); const nextVersion = Number(doc.version) + 1; const db = getD1();
    const changeNote=safeText(payload.changeNote,500)||"内容更新，重新进入草稿";
    await db.batch([
      ...
      db.prepare("INSERT INTO document_versions(document_id,version,title,content,change_note,operator_user_id,operator) VALUES(?,?,?,?,?,?,?)").bind(id, nextVersion, title, content, changeNote, ctx.userId, ctx.displayName),
      db.prepare("INSERT INTO audit_logs(document_id,dept_id,action,actor_user_id,actor,detail,request_id) VALUES(?,?,?,?,?,?,?)").bind(id, deptId, "UPDATE", ctx.userId, ctx.displayName, `更新至 V${nextVersion}：${changeNote}`, rid),
      db.prepare("UPDATE knowledge_governance_tasks SET status='IN_PROGRESS',workflow_stage='OWNER_REVISING',target_document_id=?,update_time=CURRENT_TIMESTAMP WHERE source_document_id=? AND assignee_user_id=? AND status IN ('OPEN','IN_PROGRESS')").bind(id,id,ctx.userId),
    ]);
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  const rid=requestId(request);
  try{
    const ctx=await requireApiUser();const id=Number((await context.params).id),doc=await authorizedDocument(id,ctx),deptId=Number(doc.dept_id);
    if(!canManageDepartment(ctx,deptId))throw new ApiError(403,"OWNER_TRANSFER_FORBIDDEN","仅资料所属部门管理员可转交负责人");
    const payload=await request.json() as {action?:string;ownerUserId?:number;reason?:string};
    if(payload.action!=="TRANSFER_OWNER")throw new ApiError(400,"ACTION_INVALID","不支持的资料管理操作");
    const successorId=Number(payload.ownerUserId),reason=safeText(payload.reason,500);if(!successorId)throw new ApiError(400,"SUCCESSOR_REQUIRED","请选择新的资料负责人");if(reason.length<2)throw new ApiError(400,"TRANSFER_REASON_REQUIRED","请填写责任转交原因");
    const db=getD1(),oldOwnerId=Number(doc.owner_user_id||doc.create_user_id);if(successorId===oldOwnerId)throw new ApiError(409,"OWNER_UNCHANGED","新负责人不能与当前负责人相同");
    const successor=await db.prepare(`SELECT u.id,u.display_name FROM users u JOIN user_departments ud ON ud.user_id=u.id AND ud.dept_id=? WHERE u.id=? AND u.status='ACTIVE' AND EXISTS(SELECT 1 FROM user_roles ur JOIN role_permissions rp ON rp.role_id=ur.role_id JOIN permissions p ON p.id=rp.permission_id WHERE ur.user_id=u.id AND p.code='knowledge:edit')`).bind(deptId,successorId).first<{id:number;display_name:string}>();
    if(!successor)throw new ApiError(409,"SUCCESSOR_NOT_ELIGIBLE","继承人必须是资料所属部门的在职成员，并具备资料编辑权限");
    const oldOwner=await db.prepare("SELECT display_name FROM users WHERE id=?").bind(oldOwnerId).first<{display_name:string}>();
    await db.batch([
      db.prepare("UPDATE documents SET owner_user_id=?,owner=?,update_time=CURRENT_TIMESTAMP WHERE id=?").bind(successor.id,successor.display_name,id),
      db.prepare("UPDATE knowledge_governance_tasks SET assignee_user_id=?,workflow_stage='WAITING_OWNER',update_time=CURRENT_TIMESTAMP WHERE source_document_id=? AND assignee_user_id=? AND status IN ('OPEN','IN_PROGRESS')").bind(successor.id,id,oldOwnerId),
      db.prepare("INSERT INTO audit_logs(document_id,dept_id,action,actor_user_id,actor,detail,request_id) VALUES(?,?,'OWNER_TRANSFER',?,?,?,?)").bind(id,deptId,ctx.userId,ctx.displayName,`${oldOwner?.display_name||String(doc.owner)} → ${successor.display_name}；原因：${reason}`,rid),
    ]);
    await notifyUser({userId:successor.id,type:"OWNER_TRANSFER",title:"资料责任已转交给你",content:`《${String(doc.title)}》已由 ${oldOwner?.display_name||String(doc.owner)} 转交给你维护。原因：${reason}`,documentId:id,requestId:rid,email:true});
    return ok({transferred:true,ownerUserId:successor.id,owner:successor.display_name},rid);
  }catch(error){return fail(error,rid);}
}

export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const rid = requestId(request);
  try { const ctx = await requireApiUser(); await enforceRateLimit(ctx, "feedback", 20, 60); const id = Number((await context.params).id); const doc = await authorizedDocument(id, ctx); const payload = await request.json() as { type?: string; content?: string }; const type=safeText(payload.type || "纠错", 30);const content = safeText(payload.content, 2000); if (!content) throw new ApiError(400, "VALIDATION_ERROR", "反馈内容不能为空"); const db = getD1(); const receiver=Number(doc.owner_user_id||doc.create_user_id); await db.batch([db.prepare("INSERT INTO feedback(document_id,type,content,reporter_user_id,reporter) VALUES(?,?,?,?,?)").bind(id, type, content, ctx.userId, ctx.displayName),db.prepare("INSERT INTO knowledge_governance_tasks(type,status,workflow_stage,dept_id,source_document_id,reporter_user_id,assignee_user_id,reason,detail) VALUES('DOCUMENT_FEEDBACK','OPEN','WAITING_OWNER',?,?,?,?,?,?)").bind(doc.dept_id,id,ctx.userId,receiver,type,content), db.prepare("INSERT INTO audit_logs(document_id,dept_id,action,actor_user_id,actor,detail,request_id) VALUES(?,?,?,?,?,?,?)").bind(id, doc.dept_id, "FEEDBACK", ctx.userId, ctx.displayName, content, rid)]); const delivery=await notifyUser({userId:receiver,type:"KNOWLEDGE_FEEDBACK",title:"收到知识纠错反馈",content:`${ctx.displayName} 对《${String(doc.title)}》提交了“${type}”反馈：${content}`,documentId:id,requestId:rid,email:true}); return ok({ submitted: true,governanceTaskCreated:true,notification:delivery }, rid, 201); } catch (error) { return fail(error, rid); }
}
