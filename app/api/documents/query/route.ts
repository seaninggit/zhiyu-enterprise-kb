...
export async function GET(request:Request){const rid=requestId(request);try{
  ...
  if(mode==="manage"&&!['governance:admin','page:approval_pending','page:approval_history','page:document_admin','page:lifecycle_governance'].some(permission=>hasPermission(ctx,permission)))throw new ApiError(403,"FORBIDDEN","无权访问管理资料列表");
  ...
  return ok({documents:rows.results,filterOptions,pagination:{page,pageSize,total:Number(total?.count||0),pages:Math.max(1,Math.ceil(Number(total?.count||0)/pageSize))}},rid);
