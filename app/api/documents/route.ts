...
export async function GET(request: Request) {
  ...
  try {
    ...
    const metrics=await db.prepare(`SELECT COUNT(*) total,SUM(CASE WHEN status='PENDING_DEPT_REVIEW' THEN 1 ELSE 0 END) pending,SUM(CASE WHEN parse_status='FAILED' THEN 1 ELSE 0 END) parse_failed,SUM(CASE WHEN review_due_at IS NOT NULL AND review_due_at<=date('now','+30 day') AND status='ARCHIVED_ACTIVE' THEN 1 ELSE 0 END) due_soon,SUM(CASE WHEN verification_status='VERIFIED' THEN 1 ELSE 0 END) verified FROM documents WHERE is_deleted=0 AND ${metricScope}`).bind(...(ctx.role==="SUPER_ADMIN"?[]:ctx.deptIds)).first();
