...
export async function GET(request: Request) {
  ...
  try {
    ...
    return ok({ documents: visibleDocuments, logs: logs.results, governanceTasks: governanceTasks.results, currentUser: ctx, favorites:favorites.results.map(row=>Number(row.document_id)), notifications:notifications.results, spaces:spaces.results, metrics, uploadOptions: { departments: departments.results, members: members.results } }, rid);
