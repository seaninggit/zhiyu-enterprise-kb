...
export async function GET(request: Request) {
  ...
  try {
    ...
    const logs = !hasPermission(ctx,"governance:audit")?{results:[]}:ctx.role === "SUPER_ADMIN" ? await db.prepare("SELECT * FROM audit_logs ORDER BY create_time DESC LIMIT 50").all() : await db.prepare(`SELECT * FROM audit_logs WHERE dept_id IN (${placeholders(ctx.deptIds)}) ORDER BY create_time DESC LIMIT 50`).bind(...ctx.deptIds).all();
    const canGovernFeedback=hasPermission(ctx,"governance:admin")||hasPermission(ctx,"page:feedback_governance");
    const governanceTasks = !canGovernFeedback ? { results: [] } : ctx.role === "SUPER_ADMIN"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validCategory=await db.prepare("SELECT 1 ok FROM knowledge_categories WHERE is_active=1 AND name=? AND (dept_id IS NULL OR dept_id=?) LIMIT 1").bind(category,deptId).first();
    if(!validCategory)throw new ApiError(403,"CATEGORY_FORBIDDEN","该分类不属于资料归属部门或已停用");
