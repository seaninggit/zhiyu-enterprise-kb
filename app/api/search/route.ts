...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = await request.json() as { query?: string; queryEmbedding?: unknown; useOriginal?:boolean; filters?: { category?: string; mimeType?: string; spaceId?: number } };
    const query = safeText(payload.query, 300); if (!query) throw new ApiError(400, "VALIDATION_ERROR", "请输入搜索内容");
    const db = getD1();const detected=await correctEnterpriseQuery(db,query);const correction=payload.useOriginal?{...detected,corrected:query,applied:false,reason:"已按你的选择使用原始输入检索"}:detected;const access=publishedDocumentScope(ctx,"d");let scope=access.sql;const binds:unknown[]=[...access.binds];
