...
export async function POST(request: Request) {
  ...
  try {
    ...
    const db = getD1(); let scope = "d.is_deleted=0 AND (d.status='ARCHIVED_ACTIVE' OR (d.status IN ('DRAFT','PENDING_DEPT_REVIEW') AND d.published_version IS NOT NULL))"; const binds: unknown[] = [];
    ...
    const rows = await db.prepare(`SELECT d.id,
      ...
      CASE WHEN d.status='ARCHIVED_ACTIVE' THEN d.summary ELSE COALESCE(d.published_summary,substr(d.published_content,1,180),'') END summary,d.category,d.owner,CASE WHEN d.status='ARCHIVED_ACTIVE' THEN d.version ELSE COALESCE(d.published_version,d.version) END version,
    ...
    const localVector = isValidEmbedding(payload.queryEmbedding) ? payload.queryEmbedding : undefined; const vector = localVector || (await embedTexts([query]).catch(() => []))[0]; const queryTerms = terms(query); const byDocument = new Map<number, Record<string, unknown> & { score: number; excerpt: string; relevance: number }>();
