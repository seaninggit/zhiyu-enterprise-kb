...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = await request.json() as { query?: string; filters?: { category?: string; mimeType?: string; spaceId?: number } };
    ...
    const vector = (await embedTexts([query]).catch(() => []))[0]; const queryTerms = terms(query); const byDocument = new Map<number, Record<string, unknown> & { score: number; excerpt: string }>();
    ...
    const log = await db.prepare("INSERT INTO search_logs(user_id,dept_id,query,result_count,mode) VALUES(?,?,?,?,?)").bind(ctx.userId, ctx.primaryDeptId, query, ranked.length, vector ? "HYBRID" : "KEYWORD").run();
    return ok({ searchLogId: log.meta.last_row_id, results: ranked.map(row => ({ id:row.id,title:row.title,summary:row.summary,category:row.category,owner:row.owner,version:row.version,mime_type:row.mime_type,update_time:row.update_time,verification_status:row.verification_status,space_name:row.space_name,department_name:row.department_name,excerpt:row.excerpt,score:Number(row.score.toFixed(3)) })), mode: vector ? "HYBRID" : "KEYWORD", suggestions: ranked.length ? [] : ["尝试更换关键词", "选择其他知识空间", "提交知识缺口"] }, rid);
