...
type KnowledgeDocument = {
  ...
  reviewDueAt?: string | null; createdAt: string; updatedAt: string; versions?: DocumentVersion[]; parseStatus?: string; verificationStatus?: string; spaceId?: number | null; folderId?: number | null;
};
...
export default function Home() {
  ...
  async function runSearch(){if(!query.trim()){setSearchResults(null);return;}setSearchLoading(true);try{const response=await fetch("/api/search",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({query,filters:{category:category==="全部"?undefined:category}})});const payload=await response.json();if(!response.ok)throw new Error(payload.error?.message??"检索失败");setSearchResults((payload.data.results??[]).map(normalizeDocument));setSearchLogId(Number(payload.data.searchLogId)||null);setView("library");if(!payload.data.results?.length)notify("未找到可靠结果，已记录为知识缺口");}catch(error){notify(error instanceof Error?error.message:"检索失败");}finally{setSearchLoading(false);}}
  ...
  return <div className={`enterprise-app${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
    ...
    <div className="app-main">
      ...
      {view === "admin" ? <AdminView documents={documents} metrics={metrics} governanceTasks={governanceTasks} role={currentUser.role} onUpload={() => setUploadOpen(true)} onSelect={(doc) => openDocument(doc).catch(error => notify(error instanceof Error ? error.message : "资料加载失败"))} onStatus={updateStatus} />
...
function AdminView({ documents, metrics, governanceTasks, role, onUpload, onSelect, onStatus }: { documents: KnowledgeDocument[]; metrics:Metrics; governanceTasks: GovernanceTask[]; role: string; onUpload: () => void; onSelect: (d: KnowledgeDocument) => void; onStatus: (id: number, action: "submit" | "approve" | "reject" | "archive") => void }) {
...
function PlatformView({role,notify}:{role:string;notify:(message:string)=>void}){
  ...
  const [data,setData]=useState<PlatformData|null>(null);const [loading,setLoading]=useState(true);
  ...
  return <main className="workspace"><section className="admin-heading"><div><span className="page-kicker">KNOWLEDGE OPERATIONS</span><h1>治理与洞察</h1><p>解析流水线、搜索缺口、空间目录、审批轨迹和 AI 参数统一管理。</p></div><button className="outline-action" onClick={load}>刷新数据</button></section>
...
function DocumentDrawer({ document: doc, canRestore, favorite, onClose, onFavorite, onRestore, onFeedback, onExport, onDownload, onShare, onSubscribe, onContact }: { document: KnowledgeDocument; canRestore:boolean; favorite: boolean; onClose: () => void; onFavorite: () => void; onRestore:(version:number)=>void; onFeedback: () => void; onExport: () => void; onDownload: () => void; onShare: () => void; onSubscribe: () => void; onContact: () => void }) {
