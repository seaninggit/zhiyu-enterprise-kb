...
export default function Home() {
  ...
  async function engageDocument(document: KnowledgeDocument, action: "SHARE" | "SUBSCRIBE" | "CONTACT_OWNER" | "START_WORKFLOW") {
    try {
      if (action === "SHARE") await navigator.clipboard.writeText(`${window.location.origin}/?document=${document.id}`);
      const response = await fetch("/api/engagement", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, documentId: document.id, workflowType: action === "START_WORKFLOW" ? "TRAVEL_APPLICATION" : undefined, title: action === "START_WORKFLOW" ? `依据《${document.title}》发起差旅申请` : undefined }) });
      const payload = await response.json(); if (!response.ok) throw new Error(payload.error?.message ?? "操作失败");
      notify(action === "SHARE" ? "内部链接已复制，访问时仍会校验权限" : action === "SUBSCRIBE" ? "已订阅，资料更新时将收到提醒" : action === "CONTACT_OWNER" ? `已向负责人 ${document.owner} 发起联系` : `差旅申请已提交，流程编号 ${payload.data.workflowId}`);
    } catch (error) { notify(error instanceof Error ? error.message : "操作失败"); }
  }
...
function DocumentDrawer({ document: doc, favorite, onClose, onFavorite, onFeedback, onExport, onDownload, onShare, onSubscribe, onContact, onWorkflow }: { document: KnowledgeDocument; favorite: boolean; onClose: () => void; onFavorite: () => void; onFeedback: () => void; onExport: () => void; onDownload: () => void; onShare: () => void; onSubscribe: () => void; onContact: () => void; onWorkflow: () => void }) { return <div className="drawer-backdrop" onMouseDown={onClose}><aside className="document-drawer" onMouseDown={e => e.stopPropagation()}><header><button onClick={onClose}>×</button><div><span className={`doc-status ${doc.status}`}>{statusLabel[doc.status]}</span><span>{doc.securityLevel}</span></div><h2>{doc.title}</h2><p>{doc.summary}</p></header><div className="drawer-actions"><button onClick={onFavorite}>{favorite ? "★ 已收藏" : "☆ 收藏"}</button><button onClick={onShare}>⎘ 内部链接</button><button onClick={onSubscribe}>◇ 订阅更新</button><button onClick={onContact}>@ 联系负责人</button><button onClick={onDownload}>↓ 下载原件</button><button onClick={onExport}>⇧ 导出摘要</button><button onClick={onFeedback}>! 纠错反馈</button><button onClick={onWorkflow}>→ 发起差旅申请</button></div><section className="doc-info"><div><span>知识负责人</span><b>{doc.owner}</b></div><div><span>上传人</span><b>{doc.uploader}</b></div><div><span>当前版本</span><b>V{doc.version}.0</b></div><div><span>下次复核</span><b>{doc.reviewDueAt || "未设置"}</b></div></section><article className="doc-content"><h3>文档正文</h3>{doc.content.split("\n").map((p, i) => <p key={i}>{p}</p>)}</article><section className="version-list"><h3>版本记录</h3><div><span>V{doc.version}.0</span><p><b>当前版本</b><small>{doc.uploader} 更新 · {doc.updatedAt.slice(0, 10)}</small></p></div><div><span>V{Math.max(1, doc.version - 1)}.0</span><p><b>历史版本</b><small>完成内容复核与格式修订</small></p></div></section></aside></div>; }
...
function AiPanel({ onClose, onOpen }: { onClose: () => void; onOpen: (documentId: number) => void | Promise<void> }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="ai-panel enterprise-ai" onMouseDown={e => e.stopPropagation()}><button className="close-button" onClick={onClose}>×</button><span className="ai-orb">✦</span><h2>权限感知企业问答</h2><p>仅检索你有权访问的已生效知识；回答不足时拒绝猜测，并保留版本引用。</p>{answer ? <div className="ai-answer"><span>AI</span><small>{answer.mode === "rag" ? "RAG生成回答" : answer.mode === "retrieval_only" ? "安全检索模式" : "未找到可靠依据"}</small><p>{answer.text}</p>{answer.sources.map(source => <button key={`${source.documentId}-${source.citation}`} onClick={() => onOpen(source.documentId)}><b>查看引用 {source.citation}</b>{source.title} · V{source.version}.0 · {source.department} →<small>{source.excerpt}</small></button>)}<div className="ai-followups"><button onClick={() => ask("请根据以上依据生成一份可执行的办理清单")}>生成办理清单</button><button onClick={() => fetch("/api/engagement", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "AI_HELPFUL", queryLogId: answer.queryLogId, helpful: true }) })}>有帮助</button><button onClick={() => fetch("/api/engagement", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "AI_HELPFUL", queryLogId: answer.queryLogId, helpful: false }) })}>没解决</button></div></div> : <div className="suggestions"><button onClick={() => setQuestion("差旅报销需要哪些材料？")}>差旅报销需要哪些材料？</button><button onClick={() => setQuestion("新员工第一周需要完成什么？")}>新员工第一周需要完成什么？</button></div>}{error && <p className="ai-error">{error}</p>}<div className="ai-input"><input aria-label="向企业知识库提问" value={question} onChange={e => setQuestion(e.target.value)} placeholder={history.length ? "继续追问..." : "输入问题..."} onKeyDown={e => { if (e.key === "Enter") ask(); }}/><button onClick={() => ask()} disabled={!question.trim() || loading}>{loading ? "检索中..." : "发送"}</button></div></section></div>;
}
