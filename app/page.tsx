...
function AiPanel({ documents, onClose, onOpen }: { documents: KnowledgeDocument[]; onClose: () => void; onOpen: (d: KnowledgeDocument) => void }) {
  ...
  const [question, setQuestion] = useState(""); const [loading, setLoading] = useState(false); const [answer, setAnswer] = useState<{ text: string; sources: AiSource[]; mode: string } | null>(null); const [error, setError] = useState("");
  async function ask() { if (!question.trim() || loading) return; setLoading(true); setError(""); try { const response = await fetch("/api/ai/ask", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ question }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error?.message ?? "知识问答暂不可用"); setAnswer({ text: payload.data.answer, sources: payload.data.sources, mode: payload.data.mode }); } catch (caught) { setError(caught instanceof Error ? caught.message : "知识问答暂不可用"); } finally { setLoading(false); } }
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="ai-panel enterprise-ai" onMouseDown={e => e.stopPropagation()}><button className="close-button" onClick={onClose}>×</button><span className="ai-orb">✦</span><h2>权限感知企业问答</h2><p>仅检索你有权访问的已生效知识；回答不足时拒绝猜测，并保留版本引用。</p>{answer ? <div className="ai-answer"><span>AI</span><small>{answer.mode === "rag" ? "RAG生成回答" : answer.mode === "retrieval_only" ? "安全检索模式" : "未找到可靠依据"}</small><p>{answer.text}</p>{answer.sources.map(source => { const doc = documents.find(item => item.id === source.documentId); return <button key={`${source.documentId}-${source.citation}`} disabled={!doc} onClick={() => doc && onOpen(doc)}><b>引用 {source.citation}</b>{source.title} · V{source.version}.0 · {source.department} →<small>{source.excerpt}</small></button>; })}</div> : <div className="suggestions"><button onClick={() => setQuestion("差旅报销需要哪些材料？")}>差旅报销需要哪些材料？</button><button onClick={() => setQuestion("新员工第一周需要完成什么？")}>新员工第一周需要完成什么？</button></div>}{error && <p className="ai-error">{error}</p>}<div className="ai-input"><input aria-label="向企业知识库提问" value={question} onChange={e => setQuestion(e.target.value)} placeholder="输入问题..." onKeyDown={e => { if (e.key === "Enter") ask(); }}/><button onClick={ask} disabled={!question.trim() || loading}>{loading ? "检索中..." : "发送"}</button></div></section></div>;
}
