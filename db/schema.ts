...
export const notificationDeliveries = sqliteTable("notification_deliveries", { id:integer("id").primaryKey({autoIncrement:true}), userId:integer("user_id").notNull().references(()=>users.id), documentId:integer("document_id").references(()=>documents.id), channel:text("channel").notNull(), eventType:text("event_type").notNull(), recipient:text("recipient").notNull(), status:text("status").notNull().default("PENDING"), attempt:integer("attempt").notNull().default(1), providerMessageId:text("provider_message_id").notNull().default(""), errorMessage:text("error_message").notNull().default(""), requestId:text("request_id").notNull().default(""), createTime:text("create_time").notNull().default(sql`CURRENT_TIMESTAMP`), updateTime:text("update_time").notNull().default(sql`CURRENT_TIMESTAMP`) },table=>[index("notification_deliveries_status_idx").on(table.status,table.createTime),index("notification_deliveries_user_event_idx").on(table.userId,table.eventType,table.createTime)]);
