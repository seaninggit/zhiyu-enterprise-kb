...
export const workflowRequests = sqliteTable("workflow_requests", { id: integer("id").primaryKey({ autoIncrement: true }), type: text("type").notNull(), title: text("title").notNull(), documentId: integer("document_id").notNull().references(() => documents.id), applicantUserId: integer("applicant_user_id").notNull().references(() => users.id), deptId: integer("dept_id").notNull().references(() => departments.id), status: text("status").notNull().default("SUBMITTED"), payload: text("payload").notNull().default("{}"), createTime: text("create_time").notNull().default(sql`CURRENT_TIMESTAMP`), updateTime: text("update_time").notNull().default(sql`CURRENT_TIMESTAMP`) }, (table) => [index("workflow_requests_user_status_idx").on(table.applicantUserId, table.status), index("workflow_requests_dept_time_idx").on(table.deptId, table.createTime)]);
