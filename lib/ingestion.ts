...
export async function processDocument(documentId:number) {
  ...
  try {
    ...
    if(doc.source_key) { const object=await cfg().KNOWLEDGE_FILES?.get(String(doc.source_key)); if(!object) throw new Error("SOURCE_FILE_NOT_FOUND");const bytes=await object.arrayBuffer();checksum=await sha256(bytes);const probe=new TextDecoder().decode(bytes.slice(0,8192));if(probe.includes("EICAR-STANDARD-ANTIVIRUS-TEST-FILE")){await db.prepare("INSERT INTO security_events(document_id,type,severity,detail) VALUES(?,'MALWARE','CRITICAL','检测到恶意文件特征，已阻止解析')").bind(documentId).run();throw new Error("MALWARE_DETECTED");}if(object.size>20_000_000)await db.prepare("INSERT INTO security_events(document_id,type,severity,status,detail,resolved_at) VALUES(?,'LARGE_FILE_SCAN','LOW','RESOLVED',?,CURRENT_TIMESTAMP)").bind(documentId,`大文件 ${object.size} 字节已完成内置特征扫描与校验和计算；原件继续保存在对象存储`).run();const replay={...object,arrayBuffer:async()=>bytes,text:async()=>new TextDecoder().decode(bytes)} as R2ObjectBody;if((mime.startsWith("text/")||/json|csv|xml|javascript/.test(mime))&&String(doc.extraction_method)!=="MANUAL_EDIT") extracted=mergeDistinctText(extracted,new TextDecoder().decode(bytes)); else if(!extracted) { const parsed=await aiExtract(replay,String(doc.source_name||"document"),mime); extracted=mergeDistinctText(extracted,parsed); } }
