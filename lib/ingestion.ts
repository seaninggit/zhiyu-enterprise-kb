...
import { getKnowledgeFile, KnowledgeFile } from "./knowledge-files";

type PlatformEnv = { OPENAI_API_KEY?:string; OPENAI_CHAT_MODEL?:string };
...
function mergeDistinctText(...values:string[]){const clean=values.map(value=>value.trim()).filter(Boolean);return [...new Set(clean)].join("\n\n");}
...
async function aiExtract(object:KnowledgeFile,name:string,mime:string) {
  const key=cfg().OPENAI_API_KEY; if(!key) return "";
...
export async function processDocument(documentId:number) {
  ...
  try {
    ...
    if(doc.source_key) { const object=await getKnowledgeFile(String(doc.source_key));if(!object){if(!extracted)throw new Error("SOURCE_FILE_NOT_FOUND");}else{const bytes=await object.arrayBuffer();checksum=await sha256(bytes);const probe=new TextDecoder().decode(bytes.slice(0,8192));if(probe.includes("EICAR-STANDARD-ANTIVIRUS-TEST-FILE")){await db.prepare("INSERT INTO security_events(document_id,type,severity,detail) VALUES(?,'MALWARE','CRITICAL','检测到恶意文件特征，已阻止解析')").bind(documentId).run();throw new Error("MALWARE_DETECTED");}if(object.size>20_000_000)await db.prepare("INSERT INTO security_events(document_id,type,severity,status,detail,resolved_at) VALUES(?,'LARGE_FILE_SCAN','LOW','RESOLVED',?,CURRENT_TIMESTAMP)").bind(documentId,`大文件 ${object.size} 字节已完成内置特征扫描与校验和计算；原件继续保存在对象存储`).run();const replay={...object,arrayBuffer:async()=>bytes,text:async()=>new TextDecoder().decode(bytes)};if((mime.startsWith("text/")||/json|csv|xml|javascript/.test(mime))&&String(doc.extraction_method)!=="MANUAL_EDIT") extracted=mergeDistinctText(extracted,new TextDecoder().decode(bytes)); else if(!extracted) { const parsed=await aiExtract(replay,String(doc.source_name||"document"),mime); extracted=mergeDistinctText(extracted,parsed); }} }
    const hasExtractedText=extracted.trim().length>0;const parseStatus=hasExtractedText?"COMPLETED":String(doc.ocr_status)==="FAILED"?"OCR_FAILED":mime.startsWith("image/")||mime.includes("pdf")?"OCR_REQUIRED":"NEEDS_CONTENT";
    if(!hasExtractedText) extracted=[`标题：${doc.title}`,`摘要：${doc.summary||""}`].join("\n");
    const findings=dlp(extracted);await db.prepare("UPDATE documents SET extracted_text=?,content=CASE WHEN content='' AND ?='COMPLETED' THEN ? ELSE content END,summary=CASE WHEN summary='' THEN ? ELSE summary END,parse_status=?,scan_status='CLEAN',checksum=?,dlp_findings=?,security_level=CASE WHEN ?<>'' AND security_level='INTERNAL' THEN 'SENSITIVE' ELSE security_level END,ai_index_status='PENDING',update_time=CURRENT_TIMESTAMP WHERE id=?").bind(extracted.slice(0,500000),parseStatus,extracted.slice(0,500000),extracted.replace(/\s+/g," ").slice(0,180),parseStatus,checksum,JSON.stringify(findings),findings.join(","),documentId).run();if(findings.length)await db.prepare("INSERT INTO security_events(document_id,type,severity,detail) VALUES(?,'DLP','HIGH',?)").bind(documentId,`检测到：${findings.join("、")}`).run();
    const indexed=await indexPublishedDocument(documentId).catch(async error=>{await db.prepare("UPDATE documents SET ai_index_status='FAILED' WHERE id=?").bind(documentId).run();console.error(JSON.stringify({level:"error",action:"AI_INDEX",documentId,message:error instanceof Error?error.message:"索引失败"}));return {chunks:0,embedded:false};});
    await db.prepare("UPDATE ingestion_jobs SET status='COMPLETED',stage=?,extracted_chars=?,chunk_count=?,update_time=CURRENT_TIMESTAMP WHERE id=?").bind(parseStatus==="COMPLETED"?"INDEX":"REVIEW",hasExtractedText?extracted.length:0,indexed.chunks,jobId).run();
