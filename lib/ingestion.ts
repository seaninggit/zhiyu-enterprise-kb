...
export async function processDocument(documentId:number) {
  ...
  try {
    ...
    const findings=dlp(extracted);await db.prepare("UPDATE documents SET extracted_text=?,content=CASE WHEN content='' AND ?='COMPLETED' THEN ? ELSE content END,summary=CASE WHEN summary='' THEN ? ELSE summary END,parse_status=?,scan_status=CASE WHEN scan_status='EXTERNAL_SCAN_REQUIRED' THEN scan_status ELSE 'CLEAN' END,checksum=?,dlp_findings=?,security_level=CASE WHEN ?<>'' AND security_level='INTERNAL' THEN 'SENSITIVE' ELSE security_level END,ai_index_status='PENDING',update_time=CURRENT_TIMESTAMP WHERE id=?").bind(extracted.slice(0,500000),parseStatus,extracted.slice(0,500000),extracted.replace(/\s+/g," ").slice(0,180),parseStatus,checksum,JSON.stringify(findings),findings.join(","),documentId).run();if(findings.length)await db.prepare("INSERT INTO security_events(document_id,type,severity,detail) VALUES(?,'DLP','HIGH',?)").bind(documentId,`检测到：${findings.join("、")}`).run();
