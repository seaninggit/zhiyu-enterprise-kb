...
export async function correctEnterpriseQuery(db:D1Database,query:string):Promise<QueryCorrection>{
  ...
  const rows=await db.prepare("SELECT name term FROM tags UNION ALL SELECT name FROM knowledge_categories WHERE is_active=1 UNION ALL SELECT title FROM documents WHERE is_deleted=0 ORDER BY term LIMIT 600").all<{term:string}>();
