#!/usr/bin/env python3
"""Patch AI ask route: extend regex + add semantic classifier + Agent mode."""
import re
...
# ===== 6. Add semantic fallback between platformIntent call and RAG (only if not already there) =====
if 'classifyIntent(question)' not in code:
    old_direct = '    const direct=platformIntent(question,ctx.displayName,ctx.role,ctx.isPublicViewer);\n    if(direct){'
    new_direct = '''    const hasContext=Boolean(payload.conversationId);
    ...
    if(direct){'''
    code = code.replace(old_direct, new_direct)

# ===== 7. Fix payload type for agent mode (only if not already patched) =====
if 'agent?: boolean' not in code:
    code = code.replace(
        '{ question?: string; conversationId?: number; queryEmbedding?: unknown };',
        '{ question?: string; conversationId?: number; queryEmbedding?: unknown; mode?: string; agent?: boolean };'
    )

# ===== 8. Add Agent mode after conversation creation (only if not already there) =====
if 'const agentMode' not in code:
    agent_block = '''
    ...
    if (agentMode && ctx.role !== "EMPLOYEE") {
      ...
      const srcs = result.toolCalls.map((tc: {tool:string;result:string}) => ({ documentId: 0, title: tc.tool, version: 1, department: "agent", excerpt: tc.result.slice(0, 180), score: 1 }));
